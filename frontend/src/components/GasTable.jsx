...
export default function GasTable({ messages }) {
  ...
  return <div className="gas-view">
    ...
    <p className="gas-note">Latest quote per sender and period across the selected session’s chat rooms. Best bid is the highest; best ask is the lowest. Quotes stay visible until replaced in the available message history.</p>
    ...
    {markets.map(market => <section className="gas-market" key={market.key}>
      ...
      <div className="gas-table-scroll"><table className="gas-table" aria-label={`${market.product} ${market.contract} broker quotes`}>
        ...
        <tbody>{market.brokers.map(broker => <tr key={broker.sender} title={broker.source}>
          <th scope="row">{broker.sender}<small className="gas-assumption">{broker.roomName}</small>{(broker.inferredYear || broker.inferredProduct) && <small className="gas-assumption">{[broker.inferredProduct && 'Product default', broker.inferredYear && 'Year inferred'].filter(Boolean).join(' · ')}</small>}</th>
        ...
        <tfoot><tr><th scope="row">Best Market</th><td>{price(market.bestBid)}</td><td>{price(market.bestAsk)}</td><td colSpan={2}>{market.unresolved ? 'Resolve product/year to compare' : market.bestBid > market.bestAsk ? 'Crossed quotes' : 'Across displayed brokers'}</td></tr></tfoot>
      </table></div>
    </section>)}
  </div>;
}
